...
export class HeartRateSensorState {
    ...
    updateState(data, page) {
        ...
        if (page.pageState === PageState.INIT_PAGE) {
            ...
            switch (pageNum & ~TOGGLE_MASK // check the new pages and remove the toggle bit
                ...
                case 4:
                    ...
                    this.PreviousBeat = data.getUint16(Messages.BUFFER_INDEX_MSG_DATA + 2, true);
                    break;
                case 5:
                    this.IntervalAverage = data.getUint8(Messages.BUFFER_INDEX_MSG_DATA + 1);
                    this.IntervalMax = data.getUint8(Messages.BUFFER_INDEX_MSG_DATA + 2);
                    this.SessionAverage = data.getUint8(Messages.BUFFER_INDEX_MSG_DATA + 3);
                    break;
                case 6:
                    this.SupportedFeatures = data.getUint8(Messages.BUFFER_INDEX_MSG_DATA + 2);
                    this.EnabledFeatures = data.getUint8(Messages.BUFFER_INDEX_MSG_DATA + 3);
                    break;
                case 7: {
        ...
        this.decodeDefaultHRM(new DataView(data.buffer.slice(Messages.BUFFER_INDEX_MSG_DATA + 4)));
